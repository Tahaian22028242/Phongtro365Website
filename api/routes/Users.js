...
const { prisma, PrismaClient } = require('../prismaClient');
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        // Tạo token JWT
        jwt.sign({ email: userDoc.email, id: userDoc.id }, jwtSecret, { expiresIn: '7d' }, (err, token) => {
            if (err) throw err;
...
router.get('/profile', async (req, res) => {
    ...
    if (token) {
        try {
            ...
            const { name, email, id, avatar, phone, zalo, status, createdAt, createAt, violationCount } = user;
            res.json({ name, email, id, avatar, phone, zalo, status, createdAt: createdAt || createAt, violationCount: violationCount || 0 });
        } catch (err) {
            res.json(null);
        }
    } else {
